...
//    var_dump($users);
    ...
    $questionhead = get_question_header_content(get_all_users_course_assigned($user_ids));

    echo "<div class='table-container'>";
    echo '<table  class="gridtable" id="alternatecolor">';
    ...
    if ($attendance_result['head']) {
        ...
        foreach ($attendance_result['head'] as $headr) {
            if ($previous_attandance == $headr->attendanceid) {
                echo "<th>";
                echo $days_number[date('D', $headr->sessdate)];
                echo "</th>";
            } else {
                echo "<th></th>";
                echo "<th></th>";
    ...
    if (isset($questionhead['head'])) {
        foreach ($questionhead['head'] as $qid => $header) {
            $questions = get_questions($qid);
            foreach ($questions['head'] as $id => $question) {
                echo "<th></th>";
            }
        }
    }
    ...
    if ($attendance_result['head']) {
        ...
        foreach ($attendance_result['head'] as $headr) {
            if ($previous_attandance == $headr->attendanceid) {
                echo "<th>";
                echo date('j-M-y', $headr->sessdate);
                echo "</th>";
            } else {
                echo "<th></th>";
                echo "<th></th>";
        ...
        if ($attendance_result['head']) {
            ...
            foreach ($attendance_result['head'] as $headr) {
                ...
                if ($previous_attandance == $headr->attendanceid) {
                    ...
                    if ($is_user_assigned) {
                        echo "<td>";
                        echo $user_attendance;
                        echo"</td>";
                        echo "<td>";
                        echo round(($user_attendance / $counter) * 100, 2) . '%';
                        echo"</td>";
                        echo "<td>";
                        $user_attendance = 0;
                        $counter = 1;
